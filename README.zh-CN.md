...
**与 OpenAiCompletions 的对应：** 插件将**完整对话**以单条合成 prompt（System / User / Assistant / Tool result 段落）发给 cursor-agent。cursor-agent 仅支持单条 prompt（stdin）；请求中的 `tools` / `tool_choice` 会接收但不转发。详见下方 [使用指南](#文档入口)。

---

### 📖 文档入口 — 建议从这里开始

初次使用？**使用指南**按顺序介绍架构、配置、API 与 Ironclaw 集成，一套文章讲清全貌。
...
| —           | [Provider 定义](doc/provider-definition.json) — 合并到 `~/.ironclaw/providers.json` 的 JSON。                                                                   |
...
**[Guide (EN)](doc/TECHNICAL.md)** 与 **[使用指南（中文）](doc/TECHNICAL.zh-CN.md)** 是主要技术文档：教程式结构，从简介到配置、会话、API、Ironclaw 集成。仅需 provider JSON 时使用：[provider-definition.json](doc/provider-definition.json)。
...
- **技术参考：** [doc/TECHNICAL.zh-CN.md](doc/TECHNICAL.zh-CN.md)。
